...
import { apiGet, apiSend, ApiKeyView } from "../lib/api";
...
export function ApiKeysPage() {
  ...
  const [created, setCreated] = useState<{ id: string; key: string; name: string } | null>(null);
  ...
  const rotate = useMutation({
    ...
    onSuccess: (data) => setCreated({ id: data.id, key: data.key, name: "rotated key" }),
  });

  if (keys.isLoading) return <Spinner label="Loading keys…" />;
  if (keys.error) return <EmptyState title="Could not load keys" description={keys.error.message} />;
  ...
  return (
    <div className="space-y-6">
              ...
              <tbody>
                {list.map((k) => (
                  <tr key={k.id} className="border-b border-gray-50 last:border-0">
                    ...
                    <td className="px-5 py-3 font-mono text-xs text-gray-500">{k.keyPrefix}…</td>
                    ...
                    <td className="px-5 py-3 text-right">
                      <div className="inline-flex gap-1">
                        <Button size="sm" variant="ghost" onClick={() => rotate.mutate(k.id)} title="Rotate">
                          <RotateCcw size={13} />
                        </Button>
                        {k.status === "active" && (
                          <Button
      ...
      {created && <KeyRevealModal key={created.id} data={created} onClose={() => setCreated(null)} />}
      {error && (
...
function CreateKeyModal({
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (spendCapUsd.trim() !== "" && Number.isFinite(cap) && cap > 0) scopes.spendCapUsd = cap;
      const res = await apiSend<{ id: string; key: string; name: string }>("POST", "/api/keys", {
